...
from .utils import display_tags, extract_tags_from_labels, sanitize_for_session_state_key
from .kube import is_deployment_ready, get_kubernetes_namespace, get_all_namespaces, get_kube_api_client_cached
from typing import Callable, List, Dict, Any, Optional
from . import constants
import kubernetes # For type hinting ApiClient
...
def render_resource_catalog(
    ...
    custom_obj_api: Optional[kubernetes.client.CustomObjectsApi], # Expecting CustomObjectsApi
    generic_api_client: Optional[kubernetes.client.ApiClient], # For listing namespaces
    ...
    if not available_namespaces:
        st_object.warning("Could not fetch list of available namespaces. Using default or previously selected namespace.")
        # If we can't list namespaces, we can't offer a selector.
        # We'll proceed with current_namespace_from_kube_lib which might be 'default' or from env.
    else:
        ...
        if current_namespace_from_kube_lib not in available_namespaces:
            ...
            st_object.caption(f"Current default namespace '{current_namespace_from_kube_lib}' not in listable namespaces. Defaulting selector.")
            display_namespaces_for_selector = ["default"] + [ns for ns in available_namespaces if ns != "default"]
            if "default" not in available_namespaces : display_namespaces_for_selector = available_namespaces # if default isn't even there
            
            #Try to set a sensible default for the UI if the auto-detected one is not in the list
            selected_namespace_for_ui = display_namespaces_for_selector[0] if display_namespaces_for_selector else "default"
        ...
        try:
            selected_ns_index = display_namespaces_for_selector.index(selected_namespace_for_ui)
        except ValueError: # Fallback if selected_namespace_for_ui is somehow not in the list
            selected_ns_index = 0
            selected_namespace_for_ui = display_namespaces_for_selector[0] if display_namespaces_for_selector else "default"
        ...
        if newly_selected_namespace and newly_selected_namespace != st.session_state.get('selected_k8s_namespace'):
            st.session_state.selected_k8s_namespace = newly_selected_namespace
            st.info(f"Namespace changed to: **{newly_selected_namespace}**. Refreshing list...")
    ...
    namespace_to_use = st.session_state.get('selected_k8s_namespace', current_namespace_from_kube_lib)
    ...
    if st.session_state[session_state_key_selected_resource]:
        ...
        # Pass the custom_obj_api to render_details_func if it needs to make further K8s calls
        # Assuming render_details_func is structured to accept it or uses get_kube_api_client() internally
        render_details_func(selected_resource_name) 
        st_object.markdown("---")
        if st_object.button(f"Back to {resource_type_name} List", key=f"back_to_{resource_type_name.lower()}_list_btn"):
            st.session_state[session_state_key_selected_resource] = None
            st.rerun()
    else:
        # --- List View ---
        ...
        if not custom_obj_api:
            st_object.warning(f"Kubernetes API client not initialized. Cannot fetch {resource_type_name.lower()} list.")
            return
        ...
        if resources:
            for resource_item in resources:
                ...
                item_description = item_spec.get("description", "No description provided.")
                ...
                with st_object.container(border=True):
                    ...
                    with col_button:
                        ...
                        if st.button("View Details", key=button_key):
                            st.session_state[session_state_key_selected_resource] = item_name
                            st.rerun()
        else:
            st_object.info(
