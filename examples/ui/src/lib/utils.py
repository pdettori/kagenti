...
def display_tags(st_object, tags_dict: dict):
    ...
    st_object.markdown(full_string, unsafe_allow_html=True) # unsafe_allow_html for &nbsp;
...
def sanitize_for_k8s_name(name: str) -> str:
    ...
    name = name.replace('_', constants.AGENT_NAME_SEPARATOR).replace(' ', constants.AGENT_NAME_SEPARATOR)
