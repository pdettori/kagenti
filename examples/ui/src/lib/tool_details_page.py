...
from .kube import get_custom_objects_api, get_tool_details, get_kubernetes_namespace, is_running_in_cluster # Changed to get_custom_objects_api
...
def get_mcp_client_from_session(session_key: str, mcp_url: str) -> MCPClientWrapper:
    ...
    if session_key not in st.session_state:
        logger.info(f"Creating new MCPClientWrapper for session key '{session_key}' with URL: {mcp_url}")
        ...
        if client.mcp_server_url != mcp_url: 
            logger.info(f"MCP URL changed for '{session_key}'. Re-initializing MCPClientWrapper from {client.mcp_server_url} to {mcp_url}.")
            st.session_state[session_key] = MCPClientWrapper(mcp_server_url=mcp_url) 
        else:
             logger.debug(f"Reusing existing MCPClientWrapper for session key '{session_key}' with URL {mcp_url}")
...
def render_mcp_tool_details_content(tool_k8s_name: str):
    ...
    custom_obj_api = get_custom_objects_api() # Use the correct function
    if not custom_obj_api: # Check the correct client
        st.error("Kubernetes API client (CustomObjectsApi) not available. Cannot load tool details.")
        return

    namespace = get_kubernetes_namespace()
    tool_details_data = get_tool_details(st, custom_obj_api, tool_k8s_name, namespace) # Pass correct client
    ...
    spec = tool_details_data.get("spec", {})
    mcp_server_url_from_spec = spec.get("mcpServerUrl") 
    
    mcp_server_url = None
    if mcp_server_url_from_spec:
        mcp_server_url = mcp_server_url_from_spec
        logger.info(f"Using mcpServerUrl from spec for '{tool_k8s_name}': {mcp_server_url}")
    else:
        tool_service_name = spec.get("serviceName", tool_k8s_name)
        ...
        # Allow mcpPort to override servicePort, then fallback to environment-specific default
        mcp_tool_service_port = spec.get("mcpPort", spec.get("servicePort", default_port))
        
        mcp_sse_path = spec.get("mcpSsePath", constants.DEFAULT_MCP_SSE_PATH).lstrip('/') 
        ...
        if tool_service_name:
            ...
            logger.info(f"Constructed MCP server URL for '{tool_k8s_name}': {mcp_server_url} (port: {mcp_tool_service_port}, in-cluster: {running_in_cluster})")
        else:
            st.error(f"Could not determine service name or explicit MCP URL for tool '{tool_k8s_name}'.")
            logger.error(f"MCP service name/URL for tool '{tool_k8s_name}' is missing.")
            return
            
    if not mcp_server_url:
        st.error(f"MCP Server URL for tool '{tool_k8s_name}' could not be determined. Cannot connect.")
        return
    ...
    if not mcp_client.is_last_list_tools_successful():
        ...
        if st.button(action_button_label, key=f"connect_mcp_{session_key_prefix}"):
            with st.spinner(f"Connecting to MCP server at {mcp_server_url} and listing tools..."):
                try:
                    asyncio.run(mcp_client.get_tools_from_server())
                    if mcp_client.is_last_list_tools_successful():
                        st.success(f"Successfully connected and listed tools. Found {len(mcp_client.get_cached_tools())} tools.")
                    else:
                        st.warning("Connected, but failed to list tools or no tools found.")
                    st.rerun() 
                except ConnectionError as ce: 
                    st.error(f"Connection or listing failed: {ce}")
                except Exception as e: 
                    st.error(f"Failed to connect/list tools from MCP server: {e}")
                    logger.error(f"MCP connection/list tools failed for {tool_k8s_name}: {e}", exc_info=True)
    else: 
        st.success(f"Successfully listed tools from MCP Server. Found {len(mcp_client.get_cached_tools())} tools.")
        if st.button("Refresh MCP Tools", key=f"refresh_mcp_{session_key_prefix}"):
            with st.spinner("Connecting and refreshing tools..."):
                try:
                    asyncio.run(mcp_client.get_tools_from_server())   
                    if mcp_client.is_last_list_tools_successful():
                        st.success(f"Refreshed. Found {len(mcp_client.get_cached_tools())} tools.")
                    else:
                        st.warning("Refresh attempt complete, but failed to list tools or no tools found.")
                    st.rerun()
                except ConnectionError as ce:
        ...
        if not mcp_tools_list: 
            ...
            for i, mcp_tool_data in enumerate(mcp_tools_list):
                ...
                with st.expander(f"Tool: {tool_name_on_mcp}", expanded=True if len(mcp_tools_list) == 1 else False): 
                    st.markdown(f"**Description:** {mcp_tool_data.get('description', 'N/A')}")
                    ...
                    if isinstance(input_schema, dict) and input_schema.get("type") == "object" and "properties" in input_schema:
                        ...
                        for prop_name, prop_details in input_schema.get("properties", {}).items():
                            ...
                            if prop_type == "string": default_args[prop_name] = prop_details.get("example", "your_string_value")
                            elif prop_type == "integer": default_args[prop_name] = prop_details.get("example", 0)
                            elif prop_type == "number": default_args[prop_name] = prop_details.get("example", 0.0)
                            elif prop_type == "boolean": default_args[prop_name] = prop_details.get("example", False)
                            elif prop_type == "array": default_args[prop_name] = prop_details.get("example", [])
                            elif prop_type == "object": default_args[prop_name] = prop_details.get("example", {})
                            else: default_args[prop_name] = None 
                        try:
                    ...
                    if st.button(f"Call '{tool_name_on_mcp}'", key=call_button_key):
                        try:
                            ...
                            if not isinstance(tool_args, dict):
                                st.error("Arguments must be a valid JSON object (dictionary).")
                            else:
                                with st.spinner(f"Calling tool '{tool_name_on_mcp}'..."):
                                    try:
                                        response = asyncio.run(mcp_client.execute_tool_on_server(tool_name_on_mcp, tool_args))
                                        st.markdown("**Tool Response:**")
                                        # Assuming response might be a Pydantic model, try model_dump_json
                                        ...
                                        st.error(f"Failed to call tool '{tool_name_on_mcp}': {ce}")
                                    except Exception as call_e:
                                        st.error(f"Error calling tool '{tool_name_on_mcp}': {call_e}")
                                        logger.error(f"MCP tool call failed for {tool_name_on_mcp}: {call_e}", exc_info=True)
                        except json.JSONDecodeError:
                            st.error("Invalid JSON format for arguments.")
                        except Exception as e: 
                             st.error(f"Failed to initiate tool call: {e}")

        if st.button("Clear Cached Tools & Refresh UI", key=f"clear_mcp_cache_{session_key_prefix}"):
