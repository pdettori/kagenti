...
async def _fetch_agent_card_with_resolver(
    st_object, httpx_client: httpx.AsyncClient, base_url: str, relative_card_path: str, auth_headers: dict = None
) -> AgentCard | None:
    ...
    try:
        ...
        logger.info(f"Attempting to fetch agent card from path: {relative_card_path} on base URL: {base_url}")
        ...
        logger.info(f"Successfully retrieved Agent Card from path: {relative_card_path}.")
...
async def get_effective_agent_card(st_object, base_url: str) -> AgentCard | None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not public_card:
            st_object.error("Failed to fetch public agent card. Cannot proceed with A2A interaction.")
            return None

        if public_card.supportsAuthenticatedExtendedCard:
            logger.info("Public card supports authenticated extended card. Attempting to fetch.")
            auth_headers = {"Authorization": constants.A2A_DUMMY_AUTH_TOKEN} # Use constant
            extended_card = await _fetch_agent_card_with_resolver(
                st_object, client, base_url, constants.A2A_EXTENDED_AGENT_CARD_PATH, auth_headers
            )
            if extended_card:
                logger.info("Using AUTHENTICATED EXTENDED agent card.")
                return extended_card
            else:
                logger.warning("Failed to fetch extended card, falling back to public card.")
        else:
            logger.info("Public card does not support extended card, or fetching failed. Using public card.")
        return public_card
...
def display_a2a_agent_card_details(st_object, agent_card: AgentCard):
    ...
    with st_object.expander("Capabilities & Modes", expanded=False):
        ...
        if agent_card.defaultInputModes:
            st.markdown(f"  - Default Input Modes: `{', '.join(agent_card.defaultInputModes)}`")
    ...
    st_object.caption(f"Supports Authenticated Extended Card: `{agent_card.supportsAuthenticatedExtendedCard}`")
    ...
    if isinstance(chunk_data.root, SendStreamingMessageSuccessResponse):
        ...
        if isinstance(event, Task):
            ...
            if event.status and event.status.message and event.status.message.parts:
                text_content = " ".join(
                    [p.root.text for p in event.status.message.parts if isinstance(p.root, TextPart)]
                )
                if text_content: log_message += f" | Message: {text_content}"
            is_final_event = event.status.state in ["COMPLETED", "FAILED"] if event.status else False
            ...
            # This typically isn't streamed to the main chat, but logged.
            ...
            if event.status.message and event.status.message.parts:
                ...
                if text_content: log_message += f" | Details: {text_content}"
            ...
            for part in event.artifact.parts:
                if isinstance(part.root, TextPart):
                    ...
                    log_message += f" | Data: (type: {part.root.kind}, {len(data_str)} bytes)"
            if event.lastChunk:
        ...
        unknown_chunk_msg = f"--- Received Unexpected A2A Chunk Type: {type(chunk_data.root)} ---"
        logger.warning(f"{unknown_chunk_msg}\nRaw Chunk: {chunk_data.model_dump_json(indent=2)}")
    ...
    async with httpx.AsyncClient() as httpx_client:
        try:
            ...
            if stream_response_iterator and hasattr(stream_response_iterator, 'aclose'):
                logger.info("Attempting to explicitly aclose A2A stream response iterator.")
                try:
                    ...
                    if "already running" in str(re).lower() or "no running event loop" in str(re).lower():
                        logger.warning(f"RuntimeError during explicit aclose of A2A stream (may be benign): {re}")
                    else: # Other RuntimeErrors during aclose are more concerning
                        logger.error(f"Unexpected RuntimeError during explicit aclose of A2A stream: {re}", exc_info=True)
                except Exception as ex_aclose: # Catch any other error during aclose
                    logger.error(f"Exception during explicit aclose of A2A stream: {ex_aclose}", exc_info=True)
