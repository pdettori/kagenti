...
def create_agent_build(api_instance, namespace, agent_name, url, branch_or_tag, source_path):
    ...
    with st.spinner(f"Building and deploying tool '{resource_name}'..."):
        try:
...
st.write("This page allows you to trigger the build of a new tool by providing its source URL and path.")
st.markdown("---")
...
if source_url and branch_or_tag:
    ...
    manual_subfolder_input = st.text_input(
        ...
        placeholder="e.g., tools/my-new-tool",
        key="manual_tool_source_subfolder_path"
    )
...
# Submit button
if st.button("Build New Tool", key="build_new_agent_btn"):
    if source_url and branch_or_tag and final_source_subfolder_path:
        ...
        if not agent_name:
            st.warning("Could not extract a valid tool name from the Source Subfolder Path. Please ensure it's not empty or just slashes.")
        else:
            api = load_kube_config(st)
            if api:
                namespace = os.getenv("KUBERNETES_NAMESPACE", "default")
                st.info(f"Attempting to build tool '{agent_name}' from URL: `{source_url}`, branch/tag: `{branch_or_tag}`, and path: `{final_source_subfolder_path}` in namespace: `{namespace}`.")
                ...
                st.error("Kubernetes API client not initialized. Cannot create ToolBuild.")
    else:
