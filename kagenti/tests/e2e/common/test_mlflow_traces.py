...
class TestOtelToMlflowExport:
    """
    Test OTEL Collector → MLflow trace export pipeline directly.

    This test verifies the complete export path:
    1. Send a test span to OTEL collector's OTLP endpoint
    2. OTEL collector processes and exports to MLflow
    3. Trace appears in MLflow's database

    This catches issues like:
    - OTEL collector misconfiguration
    - MLflow auth/permission errors (e.g., service account not in users table)
    ...
    def test_otel_collector_to_mlflow_export(
        ...
        """
        Send a test trace to OTEL collector and verify it appears in MLflow.

        This directly tests the OTEL → MLflow export pipeline without depending
        on agent tests to generate traces.
        """
        import uuid

        # Generate unique trace ID to identify our test trace
        test_trace_id = f"e2e-export-test-{uuid.uuid4().hex[:8]}"

        # Step 1: Send a test span to OTEL collector via kubectl exec
        # We use kubectl exec because the OTEL collector is only accessible inside the cluster
        otel_span_json = f'''{{
            "resourceSpans": [{{
                "resource": {{
                    "attributes": [
                        {{"key": "service.name", "value": {{"stringValue": "e2e-export-test"}}}},
                        {{"key": "test.id", "value": {{"stringValue": "{test_trace_id}"}}}}
                    ]
                }},
                "scopeSpans": [{{
                    "scope": {{"name": "e2e-test"}},
                    "spans": [{{
                        ...
                        "name": "e2e-export-test-span",
                        "kind": 1,
                        "startTimeUnixNano": "{int(time.time() * 1e9)}",
                        "endTimeUnixNano": "{int(time.time() * 1e9) + 1000000}",
                        "attributes": [
                            {{"key": "test.trace.id", "value": {{"stringValue": "{test_trace_id}"}}}},
                            {{"key": "mlflow.spanType", "value": {{"stringValue": "AGENT"}}}},
                            {{"key": "mlflow.traceName", "value": {{"stringValue": "e2e-export-test"}}}}
                        ]
                    }}]
                }}]
            }}]
        ...
        # Find a pod that can reach the OTEL collector
        try:
            result = subprocess.run(
                [
                    "kubectl",
            ...
            http_code = output_lines[-1] if output_lines else "unknown"

            if result.returncode != 0:
                pytest.fail(
                    f"Failed to send test trace to OTEL collector.\n"
            ...
            assert http_code in ("200", "202"), (
                f"OTEL collector returned HTTP {http_code} when receiving trace.\n"
                f"Expected 200 or 202.\n"
                f"Response: {result.stdout}\n"
                f"This may indicate OTEL collector is not accepting traces."
            )

            logger.info(
                f"Successfully sent test trace {test_trace_id} to OTEL collector"
            )

        except subprocess.TimeoutExpired:
            pytest.fail("Timeout sending test trace to OTEL collector")
        except FileNotFoundError:
            pytest.skip("kubectl not available for OTEL export test")

        # Step 2: Wait for trace to appear in MLflow
        # Give OTEL collector time to batch and export
        time.sleep(5)

        # Step 3: Check if trace appears in MLflow
        def check_for_test_trace():
            """Look for our specific test trace in MLflow."""
            try:
                traces = get_all_traces()
                for trace in traces:
                    # Check if this is our test trace by looking at span attributes
                    trace_info = trace.info if hasattr(trace, "info") else trace
                    trace_name = getattr(trace_info, "name", None) or trace_info.get(
                        "name", ""
                    )
                    if "e2e-export-test" in str(trace_name):
                        return [trace]
                return []
            except Exception as e:
                logger.warning(f"Error checking for test trace: {e}")
                return []

        try:
            found_traces = wait_for_traces(
                check_fn=check_for_test_trace,
                min_count=1,
                timeout_seconds=30,
                poll_interval=2.0,
                description=f"test trace {test_trace_id}",
            )

            assert len(found_traces) > 0, (
                f"Test trace {test_trace_id} did not appear in MLflow after 30 seconds.\n"
                "This indicates the OTEL collector → MLflow export pipeline is broken.\n"
                "Check:\n"
                "  1. OTEL collector logs for export errors\n"
                "  2. MLflow logs for ingestion errors\n"
                "  3. service-account-mlflow user exists in MLflow users table"
            )

            logger.info(
                f"OTEL → MLflow export test PASSED: trace {test_trace_id} found in MLflow"
            )

        except TimeoutError:
            pytest.fail(
                f"Test trace {test_trace_id} did not appear in MLflow.\n"
                "The OTEL collector → MLflow export pipeline is not working.\n"
                "This is likely caused by:\n"
                "  - MLflow returning 500 errors on trace ingestion\n"
                "  - service-account-mlflow user missing from MLflow users table\n"
                "  - OTEL collector OAuth2 authentication failure"
            )
