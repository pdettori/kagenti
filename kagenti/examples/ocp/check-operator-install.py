...
def main():
    ...
    parser.add_argument("namespace", help="The namespace where the Subscription exists.")
    args = parser.parse_args()

    start_time = time.time()
    
    print(f"[INFO] 1/2: Waiting for Subscription '{args.subscription}' to report its CSV...")
    csv_name = None
    ...
    if not csv_name:
        print(f"[FAIL] Timed out waiting for Subscription '{args.subscription}'.", file=sys.stderr)
        sys.exit(1)

    print(f"[INFO] 2/2: Waiting for ClusterServiceVersion '{csv_name}' to succeed...")
    while time.time() - start_time < TIMEOUT_SECONDS:
        jsonpath = "'{.status.phase}'"
        command = f"kubectl get csv {csv_name} -n {args.namespace} -o jsonpath={jsonpath}"
        ...
        if csv_phase == "Failed":
            print(f"[FAIL] CSV '{csv_name}' is in 'Failed' phase. Please investigate.", file=sys.stderr)
            sys.exit(1)
